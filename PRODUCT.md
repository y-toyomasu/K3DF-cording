...
- `R-00057`: K3ATは、実行済みTool、Policyによる拒否、実行失敗およびSystemがEvidenceから確認した成果だけから、実際に調査したAttack Pathを`observed_attack_path`として導出・表示できる。Graphは論理的なK3AT Hostを開始Nodeとし、Host → Service → Resource／Credential／Session／Objectiveの階層を扱う。実Hostname、IP、Host固有Pathまたは管理情報は表示しない。Tool ActionはNodeではなく`attempted_from`、`produced`、`used`、`continued_to`を含むEdgeとして表し、単なる時系列を因果関係にしない。Statusは`confirmed`、`executed`、`blocked`、`failed`、`incomplete`とする。Nodeは128、Edgeは256、各要素のEvidence IDは8件までとし、切捨てはSummaryに記録する。既存`attack_path`との互換を維持し、座標、選択状態または表示方向を正規Dataへ保存しない。Credentialは発見元Service、Sessionは接続先Serviceとの関係が分かる位置に置く。Dashboard用Run Recordに限り、許可済みToolで発見したUsername、Password、CookieおよびTokenの生値を詳細表示・コピーできる。生CredentialをKimiのPlanner入力または通常Tool引数へ渡さず、Credential IDによるTool利用は維持する。Git管理文書、Task、Source Code例、Test結果またはCommitへ実Credential値を記録しない。Dashboardはprivate LAN上で認証なし閲覧可能であり、到達できる利用者がCredentialを閲覧できる。K3DF Capability Graph、Strategy Brief、CTF Ground TruthおよびChallengeの正解経路とは分離し、Planner提案だけを実績として表示しない。
