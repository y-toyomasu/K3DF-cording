...
- GitHubへのPushはProduct Ownerだけが行う。Task LeadとEngineering AgentによるPush、`--amend`、rebase、force操作、およびProduct Owner受入れ前のTask Branchからlocal mainへの統合を禁止する。
- 最終受入れ前にlocal mainが進んだ場合は、必要に応じてmainをTask Branchへmergeする。Conflictまたは内容変化が発生した場合だけ影響範囲を再検証し、受入れ対象HEADが変わればProduct Ownerへ提示する。
- 最終受入れ後、Accepted Branch HEADとlocal mainが確認時点から不変でfast-forward可能な場合だけlocal mainへ統合する。不可能ならmainを変更せずTask Branchで再統合し、内容変化時だけ再検証する。Git管理外TaskはCommit対象外理由を記録し、受入れ後に`DONE`へ移せる。
