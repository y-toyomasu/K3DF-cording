...
- Product Ownerが直接編集できる正本は`AGENTS.md`だけとする。その他の正本、Task実体およびProduct Repositoryは、各Roleの通常Workflowを通じて変更する。
...
- Taskの`Source`にはRequirement、Architecture、DecisionまたはProblem / Feedbackを一つ以上記録する。`AGENTS.md`変更Taskだけは、Design Agentが提示した変更理由・内容・影響およびProduct Ownerの明示承認をSourceとして記録できる。`READY`前にSource、Dependencies、Intent、Requirements、Acceptance Criteria、Constraints、Priority、推奨モデルおよびReasoning Effortを確定する。
...
- Task Leadは、この規約の施行後に作成する新規Taskを、Engineering Agentの`CLAIM → PREPARE → IMPLEMENT → VERIFY → REPORT`が概ね30分以内のActive作業で完了する粒度として計画する。既存の公開済み、取得済みまたは`DESIGN` Taskには遡及適用しない。
...
- 30分を超える可能性が高いTaskは、Repository、責務、実装段階または検証段階の境界で、独立して検証・受入れ可能なTaskへ分割する。安全性、原子性、Rollback可能性または有効な検証を損なう分割は行わない。
...
- Requirement、Architecture、Decision、Feedback、Taskはそれぞれ`R-xxxxx`、`A-xxxxx`、`D-xxxxx`、`F-xxxxx`、`T-xxxxx`で一意に追跡する。事前永続化はTask Lead、実装・検証結果とDecision VerificationはEngineering Agentが記録する。
- `ARCHITECTURE.md`には確認済みの現行構成だけを記録する。Taskに`Architecture Impact: required after acceptance`が明記され、Product Ownerが受入れた場合だけ、Engineering Agentが受入れ後・`DONE`直前にArchitecture更新、検証、Commit、local main統合確認およびTask記録を行う。Task LeadはArchitectureを更新しない。基本設計、責務境界、外部公開面、永続化または権限境界の変更はDesign Agentの判断後に実装する。
- Roadmap Itemの実装完了表示は、対象Taskに明示されたRoadmap Itemだけを対象に、Product Owner受入れとGit変更Taskのaccepted branch headのlocal main包含を確認した後、Engineering Agentが`DONE`遷移時に行う。
